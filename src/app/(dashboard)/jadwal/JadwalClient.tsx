    ...
    <div className='space-y-6 animate-fade-in'>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-12 gap-6'>
          ...
          <div className='lg:col-span-8 space-y-4'>
            {/* Banner Guide Alert */}
            <div className='bg-blue-50/70 border border-blue-200/80 rounded-2xl p-4 flex items-start gap-3 text-xs text-blue-900 print:hidden'>
              <Info className='h-4 w-4 text-blue-600 shrink-0 mt-0.5' />
              <p className='leading-relaxed'>
                <strong>Petunjuk Pengaturan Istirahat:</strong> Arahkan kursor tetikus (hover) pada nomor{' '}
                <strong>Jam Ke</strong> di kolom pertama tabel, lalu klik tombol{' '}
                <span className='inline-flex items-center gap-1 px-2 py-0.5 rounded-md bg-emerald-100 text-emerald-800 font-bold text-[10px]'>
                  <Coffee className='h-3 w-3' /> Istirahat
                </span>{' '}
                yang muncul untuk mengubah baris tersebut menjadi jam istirahat.
              </p>
            </div>
                ...
                <table className='w-full text-xs text-left border-collapse'>
                  <thead>
                    <tr className='bg-slate-50 border-b border-slate-200 text-slate-700 font-bold text-center'>
                      <th className='p-3 border-r border-slate-200 w-16 uppercase'>JAM KE</th>
                  ...
                  <tbody>
                    {scheduleRows.map((row) => (
                      ...
                      >
                        {/* JAM KE COLUMN */}
                        <td
                          onMouseEnter={() => setHoveredJamKe(row.jamKe)}
                          onMouseLeave={() => setHoveredJamKe(null)}
                          className='p-3 border-r border-slate-200 text-center font-bold text-slate-800 relative group'
                        >
                          {row.jamKe}

                          {/* Hover Istirahat Toggle Button */}
                          <div
                            className={`absolute inset-0 bg-white/95 backdrop-blur-xs flex items-center justify-center transition-opacity print:hidden ${hoveredJamKe === row.jamKe ? 'opacity-100' : 'opacity-0 pointer-events-none'
                              }`}
                          >
                            <button
                              onClick={() => handleToggleBreak(row.jamKe)}
                              className={`px-2 py-1 rounded-lg text-[10px] font-bold gap-1 flex items-center cursor-pointer transition-all ${row.isBreak
                                  ? 'bg-slate-800 text-white hover:bg-slate-900'
                                  : 'bg-emerald-600 text-white hover:bg-emerald-700 shadow-xs'
                                }`}
                            >
                              <Coffee className='h-3 w-3' />
                              <span>{row.isBreak} Istirahat</span>
                            </button>
                          </div>
                        </td>
