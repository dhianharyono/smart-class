...
import { getSavingsSummary, getStudentLedger, addTransaction } from '@/actions/savingActions';
...
export default function TabunganClient() {
  ...
  const { data: summaries, isLoading: isSummaryLoading, isError } = useQuery<SavingsRow[]>({
    ...
    startTransition(async () => {
      try {
        ...
        if (selectedStudentId === txForm.studentId) {
          queryClient.invalidateQueries({ queryKey: ['studentLedger', selectedStudentId] });
        }

        toast.success(`Transaksi berhasil disimpan!`);
        setTxOpen(false);
        setTxForm({ studentId: '', type: 'Kredit', amount: '', description: '' });
      } catch (err: any) {
  ...
  const activeStudentName = summaries?.find((s) => s.studentId === txForm.studentId)?.name || '';
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="text-zinc-400 text-sm">
            Pantau tabungan siswa, catat setoran (Kredit), penarikan (Debit), dan riwayat mutasi dana.
          </p>
        </div>
            ...
            <Table>
              <TableHeader className="bg-zinc-900/50 border-b border-zinc-800">
                <TableRow className="border-b border-zinc-800 hover:bg-transparent">
                  ...
                  <TableHead className="w-24 text-zinc-400 font-bold">Kelas</TableHead>
                  <TableHead className="w-32 text-center text-zinc-400 font-bold">
                    Mutasi
                  </TableHead>
                  <TableHead className="w-48 text-zinc-400 font-bold">Saldo Saat Ini</TableHead>
                  <TableHead className="w-40 text-center text-zinc-400 font-bold">Aksi</TableHead>
                </TableRow>
              </TableHeader>
              <TableBody>
      ...
      <Dialog open={txOpen} onOpenChange={setTxOpen}>
        <DialogContent className="bg-zinc-900 border border-zinc-800 text-white rounded-2xl max-w-sm">
          <form onSubmit={handleTxSubmit}>
            <DialogHeader>
              <DialogTitle className="text-lg font-bold text-zinc-100">Catat Transaksi Baru</DialogTitle>
              <DialogDescription className="text-xs text-zinc-400">
                Log setoran atau penarikan uang tabungan siswa.
              </DialogDescription>
            </DialogHeader>
            ...
            <div className="space-y-4 py-4">
              ...
              <div className="space-y-1">
                <span className="text-[10px] text-zinc-500 font-bold uppercase tracking-wider block">Siswa Penerima</span>
              ...
              <div className="space-y-1.5">
                <Label className="text-zinc-300 text-sm font-semibold">Jenis Transaksi</Label>
                <Select
                  value={txForm.type}
                  onValueChange={(val) => val && setTxForm({ ...txForm, type: val as 'Kredit' | 'Debit' })}
                >
                  <SelectTrigger className="bg-zinc-950 border-zinc-800 text-white rounded-xl">
                    <SelectValue placeholder="Pilih jenis" />
                  </SelectTrigger>
                  <SelectContent className="bg-zinc-900 border-zinc-800 text-white rounded-xl">
                    <SelectItem value="Kredit">Kredit / Kas Masuk (Setoran)</SelectItem>
                    <SelectItem value="Debit">Debit / Kas Keluar (Penarikan)</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              {/* Nominal Amount */}
              <div className="space-y-1.5">
                <Label htmlFor="amount" className="text-zinc-300 text-sm font-semibold">Nominal Uang (Rupiah)</Label>
                <Input
                  id="amount"
                  type="number"
                  required
              ...
              {/* Description */}
              <div className="space-y-1.5">
                <Label htmlFor="description" className="text-zinc-300 text-sm font-semibold">Keterangan / Catatan</Label>
                <Input
                  id="description"
            ...
            <DialogFooter className="gap-2 sm:gap-0">
              ...
              >
                {isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : 'Catat Transaksi'}
              </Button>
            </DialogFooter>
          </form>
        </DialogContent>
      ...
      <Dialog open={ledgerOpen} onOpenChange={setLedgerOpen}>
        <DialogContent className="bg-zinc-900 border border-zinc-800 text-white rounded-2xl max-w-lg max-h-[85vh] flex flex-col p-6">
          <DialogHeader className="pb-4 border-b border-zinc-800">
            ...
            <DialogDescription className="text-xs text-zinc-400">
              {ledgerData ? `${ledgerData.studentName} (NIS: ${ledgerData.studentNis})` : 'Memuat data siswa...'}
            </DialogDescription>
          </DialogHeader>
          ...
          <div className="flex-1 overflow-y-auto py-4 space-y-3">
            {isLedgerLoading ? (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <span className="text-xs font-bold text-zinc-300 block">
                            {isKredit ? 'Setoran (Kredit)' : 'Penarikan (Debit)'}
                          </span>
              ...
              <div className="text-center py-12 text-zinc-650 text-xs">
                Siswa ini belum memiliki transaksi mutasi tabungan.
              </div>
            )}
          </div>
