  ...
  const statCards = [
    {
      ...
      icon: Users,
      color: 'from-emerald-500/20 to-teal-500/10 text-emerald-400 border-emerald-950',
    },
    {
    ...
    },
    {
      ...
      icon: Wallet,
      color: 'from-amber-500/20 to-orange-500/10 text-amber-400 border-amber-950',
    },
    {
      ...
      <div className="grid gap-6 md:grid-cols-3">
        ...
        <Card className="bg-zinc-900/30 border-zinc-900 rounded-2xl shadow-xl md:col-span-2">
          <CardHeader className="flex flex-row items-center justify-between">
            <div>
              <CardTitle className="text-md font-bold text-zinc-200">Tren Kas & Tabungan</CardTitle>
              <CardDescription className="text-xs text-zinc-500">Pertumbuhan total saldo tabungan kelas</CardDescription>
            </div>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <AreaChart data={stats.savingsTrend} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                  ...
                  <Tooltip 
                    ...
                    formatter={(value: any) => [formatIDR(Number(value)), 'Saldo']}
                  />
                  <Area type="monotone" dataKey="Saldo" stroke="#10b981" strokeWidth={2.5} fillOpacity={1} fill="url(#colorSaldo)" />
                </AreaChart>
              </ResponsiveContainer>
            ) : (
              <div className="flex h-full items-center justify-center text-zinc-600 text-xs">
                Belum ada data transaksi tabungan.
              </div>
            )}
          </CardContent>
        </Card>
        ...
        <Card className="bg-zinc-900/30 border-zinc-900 rounded-2xl shadow-xl">
          <CardHeader>
            <CardTitle className="text-md font-bold text-zinc-200">Distribusi Kehadiran</CardTitle>
            <CardDescription className="text-xs text-zinc-500">Rincian status kehadiran bulan ini</CardDescription>
          </CardHeader>
          <CardContent className="h-80 flex flex-col justify-between">
            <div className="h-56 relative">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart margin={{ top: 0, bottom: 0, left: 0, right: 0 }}>
                  ...
                  <Tooltip
                    contentStyle={{ backgroundColor: '#18181b', borderColor: '#27272a', borderRadius: '12px' }}
                    formatter={(value: any, name: any) => [`${value} Log Kehadiran`, name]}
                  />
                </PieChart>
              </ResponsiveContainer>
              {/* Central text displaying Rate */}
              <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none">
                <span className="text-3xl font-extrabold text-white">{stats.monthlyAttendanceRate}%</span>
                <span className="text-[10px] text-zinc-500 uppercase tracking-wider">Hadir</span>
              </div>
            </div>
            ...
            <div className="grid grid-cols-4 gap-1 text-center border-t border-zinc-900 pt-4">
              <div>
                <p className="text-xs font-bold text-emerald-400">{stats.attendanceBreakdown.Hadir}</p>
                <p className="text-[9px] text-zinc-500">Hadir</p>
              </div>
              <div>
                <p className="text-xs font-bold text-blue-400">{stats.attendanceBreakdown.Sakit}</p>
                <p className="text-[9px] text-zinc-500">Sakit</p>
              </div>
              <div>
                <p className="text-xs font-bold text-amber-400">{stats.attendanceBreakdown.Izin}</p>
                <p className="text-[9px] text-zinc-500">Izin</p>
              </div>
              <div>
                <p className="text-xs font-bold text-red-400">{stats.attendanceBreakdown.Alfa}</p>
          ...
          {stats.lowGradeNotifications.length > 0 ? (
            <div className="divide-y divide-zinc-900">
              {stats.lowGradeNotifications.map((notif) => (
                <div key={notif.gradeId} className="flex items-center justify-between py-3 group">
            ...
            <div className="flex flex-col items-center justify-center py-6 text-zinc-600 text-xs">
              <span className="text-emerald-500 font-semibold mb-1">Semua Siswa Lulus KKM!</span>
