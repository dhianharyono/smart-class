      ...
      <div className='flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between print:hidden'>
        <div>
          <h2 className='text-xl sm:text-2xl md:text-3xl font-extrabold tracking-tight text-slate-900 flex items-center gap-3'>
            <span>Roster & Data Siswa</span>
          </h2>
          <p className='text-slate-600 text-xs sm:text-sm mt-1'>
            Kelola profil lengkap seluruh siswa di kelas bimbingan Anda secara terstruktur.
          </p>
        </div>
              ...
              <Table>
                <TableHeader className='bg-slate-50/80 border-b border-slate-200'>
                  <TableRow className='border-b border-slate-200 hover:bg-transparent text-xs font-bold text-slate-700'>
                    ...
                    <TableHead className='text-slate-700 font-bold'>Foto & Nama Siswa</TableHead>
                    <TableHead className='text-slate-700 font-bold'>NIS / NISN</TableHead>
                    ...
                    <TableHead className='w-36 text-center text-slate-700 font-bold'>Aksi</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {filteredStudents.length > 0 ? (
                    filteredStudents.map((student, index) => (
                      ...
                      >
                        ...
                        <TableCell className='font-mono text-xs'>
                          <div className='font-semibold text-slate-900'>{student.nis}</div>
                          {student.nisn && (
                            <div className='text-[10px] text-slate-500'>NISN: {student.nisn}</div>
                          )}
                        </TableCell>

                        <TableCell className='font-semibold text-slate-800'>{student.className}</TableCell>

                        <TableCell>
                          <span
                            className={`px-2.5 py-0.5 rounded-full text-[11px] font-bold ${student.gender === 'L'
                    ...
                    <TableRow>
                      <TableCell colSpan={7} className='text-center py-16 text-slate-400'>
                        ...
                        <p className='text-sm font-bold text-slate-700'>Belum ada data siswa.</p>
                ...
                <div className='flex-1 overflow-y-auto p-6 space-y-6 text-xs text-slate-800'>
                  ...
                  <div className='bg-emerald-50/60 border border-emerald-200/80 rounded-2xl p-5 flex flex-col sm:flex-row items-center sm:items-start gap-4'>
                    <div className='w-24 h-24 rounded-full overflow-hidden bg-white border-2 border-emerald-300 flex items-center justify-center shrink-0 shadow-sm'>
                      {detailStudent.photo ? (
                        <img src={detailStudent.photo} alt={detailStudent.name} className='w-full h-full object-cover' />
                      ) : (
                    ...
                    <div className='flex-1 space-y-1 text-center sm:text-left'>
                      <h3 className='text-lg font-extrabold text-slate-900'>{detailStudent.name}</h3>
                      <div className='flex flex-wrap items-center justify-center sm:justify-start gap-2 text-slate-600 text-xs font-medium'>
                        <span>NIS: <strong>{detailStudent.nis}</strong></span>
                        {detailStudent.nisn && <span>&bull; NISN: <strong>{detailStudent.nisn}</strong></span>}
                        <span>&bull; Kelas: <strong>{detailStudent.className}</strong></span>
                      </div>
                      <div className='pt-1'>
                        <span
                          className={`inline-block px-2.5 py-0.5 rounded-full text-[11px] font-bold ${detailStudent.gender === 'L'
                  ...
                  <div className='space-y-3'>
                    ...
                    <div className='grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2 bg-slate-50 p-4 rounded-xl border border-slate-200/80'>
                      <div>
                        <span className='text-slate-500 font-medium block'>Tempat, Tanggal Lahir</span>
                        <span className='font-bold text-slate-900'>
                          {detailStudent.birthPlace || '-'}
                          {detailStudent.birthDate
                            ? `, ${new Date(detailStudent.birthDate).toLocaleDateString('id-ID', {
                              day: 'numeric',
                              month: 'long',
                              year: 'numeric',
                            })}`
                      ...
                      <div>
                        <span className='text-slate-500 font-medium block'>Agama</span>
                        <span className='font-bold text-slate-900'>{detailStudent.religion || '-'}</span>
                      </div>
                  ...
                  <div className='space-y-3'>
                    ...
                    <div className='grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2 bg-slate-50 p-4 rounded-xl border border-slate-200/80'>
                      <div>
                        <span className='text-slate-500 font-medium block'>Nama Ayah Kandung</span>
                        <span className='font-bold text-slate-900'>{detailStudent.fatherName || '-'}</span>
                      </div>
                      <div>
                        <span className='text-slate-500 font-medium block'>Pekerjaan Ayah</span>
                        <span className='font-bold text-slate-900'>{detailStudent.fatherJob || '-'}</span>
                      </div>
                      <div>
                        <span className='text-slate-500 font-medium block'>Nama Ibu Kandung</span>
                        <span className='font-bold text-slate-900'>{detailStudent.motherName || '-'}</span>
                      </div>
                      <div>
                        <span className='text-slate-500 font-medium block'>Pekerjaan Ibu</span>
                        <span className='font-bold text-slate-900'>{detailStudent.motherJob || '-'}</span>
                      </div>
                      {detailStudent.guardianName && (
                        <>
                          <div>
                            <span className='text-slate-500 font-medium block'>Nama Wali</span>
                            <span className='font-bold text-slate-900'>{detailStudent.guardianName}</span>
                          </div>
                          <div>
                            <span className='text-slate-500 font-medium block'>Pekerjaan Wali</span>
                            <span className='font-bold text-slate-900'>{detailStudent.guardianJob || '-'}</span>
                          </div>
                        </>
                      )}
                    </div>
                  ...
                  <div className='space-y-3'>
                    ...
                    <div className='grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2 bg-slate-50 p-4 rounded-xl border border-slate-200/80'>
                      <div>
                        <span className='text-slate-500 font-medium block'>Tanggal Masuk Sekolah</span>
                        <span className='font-bold text-slate-900'>
                          {detailStudent.entryDate
                            ? new Date(detailStudent.entryDate).toLocaleDateString('id-ID', {
                              day: 'numeric',
                              month: 'long',
                              year: 'numeric',
                            })
                      ...
                      <div>
                        <span className='text-slate-500 font-medium block'>Diterima di Tingkat Kelas</span>
                        <span className='font-bold text-slate-900'>{detailStudent.entryClass || '-'}</span>
                      </div>
                      <div>
                        <span className='text-slate-500 font-medium block'>Tahun Ajaran Pendaftaran</span>
                        <span className='font-bold text-slate-900'>{detailStudent.entryAcademicYear || '-'}</span>
                      </div>
                      <div>
                        <span className='text-slate-500 font-medium block'>Nama Sekolah Asal</span>
                        <span className='font-bold text-slate-900'>{detailStudent.previousSchool || '-'}</span>
                      </div>
                    </div>
                  </div>
                </div>
          ...
          <Dialog open={deleteOpen} onOpenChange={setDeleteOpen}>
            <DialogContent className='bg-white border border-slate-200 text-slate-900 rounded-2xl max-w-md p-6 shadow-2xl'>
              <DialogHeader className='space-y-2 text-left'>
                ...
                <DialogTitle className='text-lg font-bold text-slate-900'>Hapus Data Siswa?</DialogTitle>
                <DialogDescription className='text-xs text-slate-500 leading-relaxed'>
                  Apakah Anda yakin ingin menghapus data siswa <strong>{selectedStudent?.name}</strong>? Seluruh data presensi, nilai, dan tabungan terkait siswa ini akan ikut terhapus.
                </DialogDescription>
              </DialogHeader>
