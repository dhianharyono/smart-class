...
import { updateTeacher, deleteTeacher, createTeacher } from '@/actions/adminActions';
...
export default function ManageTeachersClient({ initialTeachers, schools }: ManageTeachersClientProps) {
  ...
  const [editRole, setEditRole] = useState<'Wali Kelas' | 'Kepala Sekolah'>('Wali Kelas');
  ...
  const [newRole, setNewRole] = useState<'Wali Kelas' | 'Kepala Sekolah'>('Wali Kelas');
        ...
        setTeachers((prev) =>
          prev.map((t) =>
            t._id === selectedTeacher._id
              ? { ...t, name: editName, email: editEmail, schoolName: editSchool, className: editClass, role: editRole }
              : t
          )
        );
        setIsEditOpen(false);
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      ...
      if (res.success) {
        toast.success(`Akun ${selectedTeacher.name} dan seluruh data kelas berhasil dihapus.`);
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-zinc-400 text-sm mt-1">
            Daftar, tambah, edit profil kelas/sekolah, dan hapus akun guru beserta data mereka di database.
          </p>
        </div>
        <Button
      ...
      <div className="flex flex-col md:flex-row gap-4 justify-between items-center bg-zinc-900/20 p-4 border border-zinc-900 rounded-2xl backdrop-blur-sm shadow-md w-full">
        <div className="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
          ...
          <div className="relative w-full sm:w-64">
            ...
            >
              <option value="" className="bg-zinc-900 text-zinc-300">Semua Sekolah</option>
              {schools.map((s) => (
                <option key={s._id} value={s.name} className="bg-zinc-900 text-zinc-300">
                  {s.name}
                </option>
              ))}
            </select>
              ...
              <table className="w-full text-left text-sm text-zinc-300 border-collapse">
                ...
                <tbody className="divide-y divide-zinc-900">
                  {filteredTeachers.map((teacher) => (
                    <tr key={teacher._id} className="hover:bg-zinc-900/10 transition-colors group">
                      <td className="py-4 px-6">
                        <div className="flex flex-col">
                          ...
                          <div className="flex items-center gap-2 mt-0.5">
                            ...
                            <span className={`px-1.5 py-0.5 border text-[9px] font-semibold rounded-md uppercase tracking-wider ${
      ...
      {isEditOpen && selectedTeacher && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="bg-zinc-900 border border-zinc-800 w-full max-w-md rounded-2xl shadow-2xl p-6 relative animate-in zoom-in-95 duration-200">
            ...
            <h3 className="text-lg font-bold text-white mb-4">Edit Profil Wali Kelas</h3>
            <form onSubmit={handleEditSubmit} className="space-y-4">
              ...
              <div className="space-y-1">
                ...
                >
                  <option value="Wali Kelas" className="bg-zinc-900 text-zinc-300">Wali Kelas</option>
                  <option value="Kepala Sekolah" className="bg-zinc-900 text-zinc-300">Kepala Sekolah</option>
                </select>
              </div>
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  >
                    ...
                    {schools.map((s) => (
                      <option key={s._id} value={s.name} className="bg-zinc-900">
                        {s.name}
                      </option>
                    ))}
                  </select>
          ...
          <div className="bg-zinc-900 border border-zinc-800 w-full max-w-md rounded-2xl shadow-2xl p-6 relative animate-in zoom-in-95 duration-200">
            ...
            <div className="space-y-3 mb-6">
              <p className="text-sm text-zinc-300">
                Apakah Anda yakin ingin menghapus akun guru <strong className="text-white">{selectedTeacher.name}</strong>?
              </p>
              <div className="p-3 bg-rose-950/20 border border-rose-950/40 rounded-xl text-xs text-rose-300 space-y-1">
                <p className="font-semibold text-rose-200">⚠️ PENTING: Tindakan ini bersifat permanen!</p>
                <p>Menghapus akun ini juga akan menghapus secara bersih data berikut dari database:</p>
          ...
          <div className="bg-zinc-900 border border-zinc-800 w-full max-w-md rounded-2xl shadow-2xl p-6 relative animate-in zoom-in-95 duration-200">
            ...
            {schools.length === 0 ? (
              <div className="text-center py-6 text-zinc-400 text-sm space-y-3">
                ...
                <p className="text-xs">Harap tambahkan sekolah terlebih dahulu di halaman Kelola Sekolah sebelum membuat wali kelas.</p>
                <Button
                  onClick={() => setIsCreateOpen(false)}
