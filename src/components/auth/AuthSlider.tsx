...
export default function AuthSlider({
  ...
  return (
    <div className='min-h-screen w-full bg-white relative overflow-x-hidden font-sans flex flex-col md:flex-row md:h-screen md:overflow-hidden'>
      {!!process.env.NEXT_PUBLIC_RECAPTCHA_SITE_KEY && (
        <Script
      ...
      {/* Desktop Back Button (Floating top-left pill with adaptive theme) */}
      <div className='hidden md:block fixed top-6 left-6 lg:top-8 lg:left-8 z-50'>
        <Link
          href='/'
          className={`inline-flex items-center gap-2.5 px-4 py-2.5 rounded-full text-xs font-semibold transition-all duration-300 backdrop-blur-md group shadow-xs hover:shadow-md ${
            mode === 'signin'
              ? 'bg-white/95 hover:bg-white text-slate-700 hover:text-emerald-700 border border-slate-200/80 hover:border-emerald-300'
              : 'bg-white/15 hover:bg-white text-white hover:text-emerald-800 border border-white/25 hover:border-white shadow-sm'
          }`}
        >
          <ArrowLeft
            className={`h-4 w-4 transition-transform duration-200 group-hover:-translate-x-1 ${
              mode === 'signin'
                ? 'text-emerald-600'
                : 'text-emerald-200 group-hover:text-emerald-800'
            }`}
          />
      ...
      {/* Mobile Header Bar */}
      <div className='md:hidden flex items-center justify-between px-4 py-3 bg-white/95 backdrop-blur-md border-b border-slate-100 sticky top-0 z-40'>
        <Link
          href='/'
          className='inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-slate-100 text-slate-700 hover:text-slate-900 border border-slate-200/80 text-xs font-semibold transition-all'
        >
          <ArrowLeft className='h-3.5 w-3.5 text-emerald-600' />
          <span>Kembali ke Beranda</span>
        </Link>
        <div className='inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-emerald-50 border border-emerald-200/60 text-[11px] font-bold text-emerald-700'>
      ...
      {/* ------------------------------------------------------------------ */}
      <div
        className={`w-full md:w-1/2 min-h-screen md:min-h-0 md:h-screen overflow-y-auto px-6 py-8 sm:px-12 sm:py-12 lg:px-16 xl:px-24 flex-col justify-between transition-all duration-300 bg-white relative z-10 ${
          mode === 'signin' ? 'flex' : 'hidden md:flex opacity-90'
        }`}
      >
        <div className='hidden md:block h-12 lg:h-16' />
        <div className='max-w-md mx-auto w-full my-auto space-y-5 sm:space-y-6 text-center sm:text-left py-6'>
          {/* Header Icon & Title */}
      ...
      >
        <div className='hidden md:block h-12 lg:h-16' />
        <div className='max-w-md mx-auto w-full my-auto space-y-4 sm:space-y-5 text-center sm:text-left py-6'>
          {/* Header Icon & Title */}
          <div className='flex flex-col items-center text-center'>
            <div className='h-11 w-11 sm:h-12 sm:w-12 rounded-2xl bg-gradient-to-tr from-emerald-600 to-teal-500 text-white flex items-center justify-center mb-2.5 sm:mb-3 shadow-lg shadow-emerald-500/20'>
              <BookOpen className='h-5 w-5 sm:h-6 sm:w-6' />
      ...
      {/* ------------------------------------------------------------------ */}
      <motion.div
        className='hidden md:flex absolute top-0 bottom-0 left-0 w-1/2 h-full z-30 p-10 lg:p-16 xl:p-20 flex-col justify-center items-center text-center overflow-hidden shadow-2xl bg-gradient-to-br from-emerald-700 via-emerald-600 to-teal-800 text-white'
        initial={false}
        animate={{ x: mode === 'signin' ? '100%' : '0%' }}
        transition={{ type: 'spring', stiffness: 200, damping: 26 }}
      >
        {/* Decorative background curves matching site branding */}
        <div className='absolute -top-24 -left-24 w-[450px] h-[450px] rounded-full border border-white/10 pointer-events-none' />
        <div className='absolute -top-44 -left-44 w-[650px] h-[650px] rounded-full border border-white/5 pointer-events-none' />
        <div className='absolute -bottom-32 -right-32 w-[520px] h-[520px] rounded-full border border-white/15 pointer-events-none' />
        <div className='absolute -bottom-56 -right-56 w-[750px] h-[750px] rounded-full border border-white/5 pointer-events-none' />
        <div className='absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[550px] h-[550px] bg-teal-400/20 rounded-full blur-3xl pointer-events-none' />
